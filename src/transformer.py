...
class Transformer(nn.Module):
    ...
    def __greedy_decode(
        ...
        for _ in range(max_length):
            ...
            dec_output: torch.Tensor = self.output_linear(
                dec_output
            )  # (batch, seq_len, vocab_size)

            # Get the logits for the last time step
            logits: torch.Tensor = dec_output[
                :, -1, :
            ...
            tgt_input: torch.Tensor = torch.cat(
                [tgt_input, next_token], dim=1
        ...
        generated_sequence: torch.Tensor = tgt_input[
            :, 1:
    ...
    def __beam_search_decode(
        ...
        for _ in range(max_length):
            ...
            beam: list[tuple[torch.Tensor, float]] = sorted(
    ...
    def __sampling_decode(
        ...
        **kwargs,
        ...
        for _ in range(max_length):
            ...
            probs: torch.Tensor = torch.softmax(scaled_logits, dim=-1)
    ...
    def __top_k_sampling_decode(
        ...
        for _ in range(max_length):
            ...
            next_token: torch.Tensor = topk_indices.gather(1, next_token)
    ...
    def __top_p_sampling_decode(
        ...
        for _ in range(max_length):
            ...
            sorted_probs: torch.Tensor = sorted_probs / sorted_probs.sum(
                dim=-1, keepdim=True
            )
    ...
    def __contrastive_decode(
        ...
        **kwargs,
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input: torch.Tensor = tgt_input.repeat_interleave(
                k, dim=0
            )  # Shape: (k, seq_len)
            next_tokens: torch.Tensor = topk_indices.view(-1, 1)  # Shape: (k, 1)
            y_candidates: torch.Tensor = torch.cat(
                [expanded_tgt_input, next_tokens], dim=1
            )  # Shape: (k, seq_len + 1)

            # Pass each candidate through the decoder
            dec_outputs_candidate: torch.Tensor = self.decoder(
                y_candidates, enc_output.repeat(k, 1, 1)
            )

            # Extract hidden states
            h_v: torch.Tensor = dec_outputs_candidate[
                :, -1, :
            ]  # Shape: (k, hidden_size)
            h_j: torch.Tensor = dec_outputs_candidate[
                :, :-1, :
            ]  # Shape: (k, seq_len, hidden_size)

            # Normalize hidden states
            h_v_norm: torch.Tensor = F.normalize(h_v, dim=-1)  # Shape: (k, hidden_size)
            h_j_norm: torch.Tensor = F.normalize(
                h_j, dim=-1
            )  # Shape: (k, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim: torch.Tensor = torch.bmm(h_j_norm, h_v_norm.unsqueeze(-1)).squeeze(
                -1
            )  # Shape: (k, seq_len)
            ...
            best_token: torch.Tensor = topk_indices[:, best_idx].unsqueeze(
                0
            )  # Shape: (1, 1)
